...
def node_var(dimensions, n):
    ...
    temp[n] = -1
    res = torch.reshape(torch.arange(0, dimensions[n], dtype=torch.float32), tuple(temp))
...
def node_div(x1, x2, dims=[]):
    return torch.where(x2 != 0.0, torch.div(x1, x2), torch.tensor(0.0, dtype=torch.float32))
...
def node_log(x1, dims=[]):
    return torch.where(x1 > 0, torch.log(x1), torch.tensor(-1.0, dtype=torch.float32))
...
def node_mod(x1, x2, dims=[]):
    return torch.where(x2 != 0.0, torch.fmod(x1, x2), torch.tensor(0.0, dtype=torch.float32))
...
def node_pow(x1, x2, dims=[]):
    return torch.where(x1 != 0, torch.pow(torch.abs(x1), torch.abs(x2)), torch.tensor(0.0, dtype=torch.float32))

def node_sign(x1, dims=[]):
    return torch.sign(x1)

def node_sqrt(child1, dims=[]):
    return torch.where(child1 > 0, torch.sqrt(child1), torch.tensor(0.0, dtype=torch.float32))
...
def node_sstep(x1, dims=[]):
    ...
    return torch.add(torch.sub(torch.mul(3.0 * _domain_delta, x2),
                  ...
                  torch.tensor(_min_domain, dtype=torch.float32))
...
def node_stack(nums, dimensions, edims):
    return torch.stack([torch.full(dimensions[:edims], float(carvar), dtype = torch.float32) for carvar in nums], dim = edims)
    ...
    def get_tensor(self, engref):
        ...
        if self.terminal:
            if self.value == 'scalar':
                ...
                if args == 1:
                    return torch.full(engref.target_dims, self.children[0], dtype=torch.float32)
                else:
                    ...
                    return node_stack(extend_children, engref.target_dims, engref.effective_dims)
...
class Engine:
    ...
    def __init__(self,
        ...
        self.device = set_device() # Check for available devices
        ...
        if isinstance(target, str):
            ...
            self.target = tree.get_tensor(self).float() # cast to an int tensor
    ...
    def final_transform_domain(self, final_tensor):
        final_tensor = torch.where(torch.isnan(final_tensor), torch.tensor(0.0, dtype=torch.float32), final_tensor)
    ...
    def calculate_tensors(self, population):
        ...
        batch = 10
        loops = math.ceil(len(population) / batch)
        lastl = len(population) - ((loops - 1) * batch)

        for l in range(loops):
            iterations = lastl if l == loops - 1 else batch
    ...
    def write_stats_to_csv(self, data):
        ...
        with open(fn, mode='w', newline='') as file:
            ...
            for d in data:
                ...
                ind += 1
            file.write("[resolution, seed, max_dep, initialization time, tensor time, fitness time, total, engine time]\n")
            fwriter.writerow([self.target_dims[0], self.experiment.seed, self.max_tree_depth, self.elapsed_init_time,
...
# TODO list
# we need to solve the batch size to offload stuff to the GPU
# provide engine wrapper for changing terminal set and func set
# simplify fitness function process
# we can't write custom ops ??!?!?
# improve more verbose debug options
# add more ops like identity
# effective dims defaults to domain size in Terminal_Set()
# add min_pop_depth warning while reading initial pop from file for
# ignore stop criteria fitness if no target
# take out minimizing/maximization to deal with multi-objetive otimization
# min and max domains should be a list
# do parameter for experiment directory
# default device to None to use both cpu and gpu
# save_to:file is only used in fitness function, making it redundant, it could be used to define interval to write statistics
# precious state will have to be overhauled
# add more genetic ops
# self.engine_rng.random() < 0.05: make this an engine parameter
# redundant dim in fit function, = len(resolution)
# the fraction node in TF is not prepared for negative numbers (problem in lerp, althoigh not really a problem...)
# we should initialize TF seed in the beggining in case we use uniform sampling
